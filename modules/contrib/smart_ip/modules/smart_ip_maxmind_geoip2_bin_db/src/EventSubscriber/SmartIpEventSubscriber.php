...
class SmartIpEventSubscriber extends SmartIpEventSubscriberBase {
  ...
  /**
   * {@inheritdoc}
   * @throws \MaxMind\Db\Reader\InvalidDatabaseException|\GeoIp2\Exception\AddressNotFoundException
   * @throws \Exception
   */
  ...
  public function formSettings(AdminSettingsEvent $event) {
    ...
    $form['smart_ip_data_source_selection']['maxmind_geoip2_bin_db_user_account'] = [
      ...
      '#description' => t(
        'Enter your MaxMind user account ID. This is required for
        lite version.'
      ),
      '#default_value' => $config->get('user_account'),
      '#size' => 30,
      '#states' => [
        'visible' => [
          ':input[name="smart_ip_data_source"]' => ['value' => self::sourceId()],
          ':input[name="maxmind_geoip2_bin_db_version"]' =>['value' => MaxmindGeoip2BinDb::LITE_VERSION],
          ':input[name="maxmind_geoip2_bin_db_auto_update"]' => ['value' => '1'],
        ],
      ],
    ];
    $form['smart_ip_data_source_selection']['maxmind_geoip2_bin_db_license_key'] = [
      ...
      '#description' => t(
        "Enter your MaxMind GeoIP2 account's license key (view your
  ...
  public function validateFormSettings(AdminSettingsEvent $event) {
    ...
    if ($formState->getValue('smart_ip_data_source') == self::sourceId()) {
      ...
      if ($autoUpdate && $formState->getValue('maxmind_geoip2_bin_db_version') == MaxmindGeoip2BinDb::LITE_VERSION && $formState->isValueEmpty('maxmind_geoip2_bin_db_user_account')) {
        $formState->setErrorByName('maxmind_geoip2_bin_db_user_account', t('Please provide MaxMind GeoIP2 user account ID.'));
      }
      if ($autoUpdate && $formState->getValue('maxmind_geoip2_bin_db_version') == MaxmindGeoip2BinDb::LITE_VERSION && $formState->isValueEmpty('maxmind_geoip2_bin_db_license_key') || $formState->getValue('maxmind_geoip2_bin_db_version') == MaxmindGeoip2BinDb::LINCENSED_VERSION && $formState->isValueEmpty('maxmind_geoip2_bin_db_license_key')) {
      ...
      if ($autoUpdate || (!$autoUpdate && $formState->isValueEmpty('maxmind_geoip2_bin_db_custom_path'))) {
        if (empty(PrivateStream::basePath())) {
          $formState->setErrorByName('maxmind_geoip2_bin_db_auto_update', t(
              'Your private file system path is not yet configured.
            ...
            if ($autoUpdate) {
              if (empty($privateFolder)) {
                $message = t('There is an @issue in Drupal private file system path.', [
                  '@issue' => Link::fromTextAndUrl(t('issue'), Url::fromRoute('system.status', [], ['fragment' => 'error']))->toString(),
                ]);
              }
              else {
                $message = t(
                  'Initially you need to manually download the @file and
  ...
  public function submitFormSettings(AdminSettingsEvent $event) {
    ...
    if ($formState->getValue('smart_ip_data_source') == self::sourceId()) {
      ...
      $config->set('version', $formState->getValue('maxmind_geoip2_bin_db_version'))
        ...
        ->set('user_account', $formState->getValue('maxmind_geoip2_bin_db_user_account'))
