...
abstract class DatabaseFileUtilityBase implements DatabaseFileUtilityInterface {
   ...
   * @return string
   */
  public static function getPath(bool $autoUpdate, string $customPath): string {
   ...
   * {@inheritdoc}
   */
  public static function needsUpdate(int $lastUpdateTime, bool $autoUpdate, int $frequency): bool {
    if ($autoUpdate) {
  ...
  protected static function requestDatabaseFile($url, $file, $sourceId) {
    ...
    if (!file_exists($destination)) {
      ...
      $filesystem->prepareDirectory($destinationRealPath);
    }
    if (file_exists($destination)) {
      if (file_exists($source)) {
        // Remove old temporary database download directory if still exist.
        $filesystem->deleteRecursive($source);
      }
      ...
      $savedArchivePath = $filesystem->saveData($data, $archivePath, FileSystemInterface::EXISTS_REPLACE);
      if (!$savedArchivePath) {
      ...
      $sourceScanFiles = $filesystem->scanDirectory($sourceRealPath, "/^$file$/");
      foreach ($sourceScanFiles as $sourceScanFile) {
        if ($sourceScanFile->filename == $file) {
          $filesystem->move($sourceScanFile->uri, $sourceFilePath, FileSystemInterface::EXISTS_REPLACE);
          break;
        }
      }
      // Verify it.
      ...
      if ($filesystem->move($sourceFilePath, $destinationFilePath) === FALSE) {
