...
class SmartIpAdminSettingsForm extends ConfigFormBase {
  ...
  protected function getEditableConfigNames(): array {
    ...
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::GET_CONFIG_NAME);
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if (isset($_SERVER['HTTP_X_AKAMAI_EDGESCAPE'])) {
      $esHeaderStat = $this->t('available');
      $fallbackDataSource[] = 3;
    }
    else {
      $esHeaderStat = $this->t('not available');
    }
    $form['smart_ip_data_source_selection']['smart_ip_fallback_data_source'] = [
      ...
      '#options' => [
        ...
        $this->t(
          'Cloudflare IP Geolocation: your website must be using
        ...
        $this->t(
          'Akamai EdgeScape: your website must be using Akamai CDN and
          "EdgeScape Geolocation" option must be enabled at your @settings
          (@status)', [
          '@settings' => Link::fromTextAndUrl($this->t('Akamai settings'), Url::fromUri('https://control.akamai.com/apps/auth'))->toString(),
          '@status'   => $esHeaderStat,
        ]),
      ],
    ];
    ...
    $form['smart_ip_preferences']['smart_ip_excluded_ips'] = [
      '#title'       => $this->t('IP addresses to exclude from look up'),
      '#type'        => 'textarea',
      '#rows'        => 5,
      '#description' => $this->t(
        'Specify the IP addresses to exclude from being looked up. Enter
        one IP address per line.'),
      '#default_value' => $config->get('excluded_ips'),
    ];
    ...
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::DISPLAY_SETTINGS);
    $form = $event->getForm();
  ...
  public static function manualUpdate() {
    ...
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::MANUAL_UPDATE);
  }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $event->setForm($form);
    $event->setFormState($form_state);
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::VALIDATE_SETTINGS);
    $form = $event->getForm();
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $event->setForm($form);
    $event->setFormState($form_state);
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::SUBMIT_SETTINGS);
    $form = $event->getForm();
  }
