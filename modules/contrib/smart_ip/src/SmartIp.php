...
class SmartIp {
   ...
   * @throws \Exception
   */
  public static function query(string $ip = NULL): array {
    $ip = trim((string) $ip);
    if (empty($ip)) {
      $ip = \Drupal::request()->getClientIp();
    }
    // Skip querying if IP address is in exclude list
    $excludedIps = \Drupal::configFactory()->get('smart_ip.settings')->get('excluded_ips');
    ...
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::QUERY_IP);
    ...
    \Drupal::service('event_dispatcher')->dispatch($event, SmartIpEvents::DATA_ACQUIRED);
    $result = $location->getData(FALSE);
    $results[$ip] = $result;
    return $result;
  }
   ...
   */
  public static function isUserDebugMode(int $uid = NULL): bool {
    $inDebugMode = FALSE;
    $config      = \Drupal::config('smart_ip.settings');
   ...
   * @throws \Exception
   */
  public static function updateUserLocation(): void {
   ...
   *   Location details.
   */
  private static function userLocationFallback(array &$location): void {
    if (!isset($location['country']) && !isset($location['countryCode']) &&
      ...
      elseif (isset($_SERVER['HTTP_X_AKAMAI_EDGESCAPE'])) {
        ...
        if (isset($esHeader['country_code'])) {
          \Drupal::moduleHandler()->loadInclude('smart_ip', 'inc', 'includes/smart_ip.country_list');
   ...
   * @return bool
   */
  public static function checkAllowedPage(): bool {
   ...
   * @return int
   */
  public static function ipAddressVersion(string $ipAddress = NULL) {
